...
void canny_edge_detection(char* src, char* dst) {
	...
	time_two = clock();
	//fprintf(stderr, "%s %f %s" ,"Allocate read took:", (((double) (time_two - time_one)) / CLOCKS_PER_SEC), "\n");
	...
	time_three = clock();
	//fprintf(stderr, "%s %f %s" ,"Execute read and setup write took:", (((double) (time_three - time_two)) / CLOCKS_PER_SEC), "\n");
...
void convolution(png_bytep *input, png_bytep *output, float *kernel, const unsigned width, const unsigned height, const int z, const bool normalize) {
	...
	if (normalize) {
			...
			float pixels = new float[(pixels_width) * (pixels_height)];
